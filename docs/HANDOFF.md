...
- **🔴 F3 — gate blind-spot: silent no-op classes (PROMOTE).** Named off-token utilities mapping to a **cleared namespace** (`rounded-2xl/3xl`) produce no CSS and are **NOT flagged** → a real feature (rounded cards) silently broke while `npm run check` stayed green. Fix: flag named utilities outside the system's defined scale, not just `[...]` arbitraries.
