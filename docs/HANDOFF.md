...
**v1 is COMPLETE (M0–M6).** Plans/specs live in `docs/superpowers/`. **Fast-follows F2 + F3 DONE (2026-06-19)** — the two "silently breaks features" items are closed (F3 = off-token-scale check; F2 = one-step non-color extension, also fixes F4). Remaining fast-follows: multi-model run, brownfield baseline (F6), align the `check` script to the test suite (F5), 5 more themes, bezier/shadow editors, pick-anywhere, Claude skill, stylelint.
