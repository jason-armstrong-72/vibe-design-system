...
- **Generator:** `scripts/generate-catalog.ts` (run by `npm run tokens`, or a sibling `npm run catalog` the plan decides) emits a markdown catalog — either a new `design-system.components.md` or a section appended into `design-system.md` (plan decides; leaning a dedicated file the AGENTS block points to).
